...
| Footer | The logo, a **Stay Connected** block (Instagram, Facebook, LinkedIn and YouTube as icon-only links, all pointing at the Australian accounts — `mumbaidabbawalaau`; **TikTok is a dimmed `<span>` placeholder** — no anchor until a URL exists, since a link with no destination is a dead control on a live page. Activate it by wrapping it in the same `<a href … aria-label>` the others use and dropping `.foot__pending`. X was removed: the only account was India-only, and a Perth page pointing there was worse than not linking at all) and **Contact** with `hello@mumbaidabbawala.com.au`. No phone number anywhere, by design |
