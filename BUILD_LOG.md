...
## 2026-08-26 — TikTok link in; the footer row is now five live links

Supplied as `https://www.tiktok.com/@mumbaidabbawalaau` with TikTok's `?_r=1&_t=…`
share pair appended.

**Published without the query pair.** `_t` is a share-session token generated by
the sender's own TikTok app when they hit Share, and `_r=1` marks it as a share
redirect — neither is part of the address, and putting a personal share token on
a link every visitor loads is not something to ship. Clean form:
`https://www.tiktok.com/@mumbaidabbawalaau`.

The placeholder `<span class="foot__pending">` became a real anchor with the same
`target="_blank" rel="noopener"` and `aria-label="Mumbai Dabbawala on TikTok"` the
other four carry, and both `.foot__pending` rules came out with it.

### A regression I caught in the diff rather than shipping

Removing the dead CSS, I deleted the block

```css
.foot__links a,
.foot__links .foot__pending { /* the 38px hit-target block */ }
```

…and left `.foot__links a,` dangling onto the *next* rule, which was
`.foot__pending { opacity: 0.42; cursor: default }`. That would have **faded every
footer icon to 42% and given them `cursor: default`** — a live link that looks
disabled. Reverted the file and redid it: `.foot__links a` keeps its own target
block, the dimming rule is gone entirely.

The lesson is the cheap one: a selector list is not a safe place to delete half of.
Reading `git diff` before believing the edit is what caught it, not the test after.

### On verifying the URL
**I could not confirm the account exists.** TikTok returns 200 with a generic
`<title>TikTok</title>` app shell to anything without a browser session — no
`uniqueId`, no `nickname`, and the handle is not even present in the 106KB body.
Consistent with the standing note that a 200 from TikTok proves nothing. It is
published on the user's authority; the presence of a genuine `_t` share token is
at least evidence it came off the real profile's share sheet.

### Verified
- Footer row: **5 marks, 5 anchors, 0 placeholder spans.** Every one 38×38, `opacity: 1`, `cursor: pointer`, brand red `#ED3237` — checked because the botched cut above would have shown up exactly here.
- All five hrefs read back correctly with their aria-labels; TikTok carries `target="_blank" rel="noopener"`.
- No `foot__pending` left in markup or styles.

### Correction, same session

I stripped the share token from the `href` and then **wrote its value into an HTML
comment on the same page** — which publishes it just as surely, since comments
ship to the client. Verified it was live: `curl … /index.html | grep -c` returned
1. Removed from the markup and from this log's own first line (the repo is
public), keeping the reasoning and dropping the value.

Worth stating as the rule, because the mistake is an easy one: **a comment
explaining why something was removed must not contain the thing.**

### Also found: the dev docs are publicly served
`new-WT` deploys as a plain static directory, so everything in it is fetchable:

| | |
| --- | --- |
| `BUILD_LOG.md` | **200** |
| `CONTEXT.md` | **200** |
| `scripts/generate-assets.mjs` | **200** |
| `README.md`, `.gitignore` | 404 (Vercel hides these two by default) |

**No credentials are exposed** — checked rather than assumed: no key-shaped
string in any of them, and the script only *reads* `FAL_KEY` from
`process.env` / `.env.local`, which is gitignored and never deployed. The two
`CONTEXT.md` grep hits for "token" are the word used for CSS custom properties.
...
## 2026-08-27 — Docs brought current, and a split log merged back

### The log had quietly forked

There were **two** build logs: this one at the repo root, and
`new-WT/BUILD_LOG.md`. Not duplicates — a genuine fork. Five entries from 26
August existed **only** in the new-WT copy and had never reached this file:

- Mobile: smaller hero type, smaller button, full countdown
- The countdown moves into the copy column on mobile
- The run's mechanic ported to mobile, without the pin
- Stage labels and notes off the mobile run
- Mobile run: the dabba slowed down

Cause: for those five tasks I ran `cat >> BUILD_LOG.md` while the shell's working
directory was `new-WT/`, which created a second file instead of appending to this
one. The giveaway was that the file had no `# Build Log` heading — it began
mid-stream with `---`. It then went stale on 26 August while this one kept going,
and it was **publicly served** (`/BUILD_LOG.md` returned 200 from the CDN), so the
stale fork was the one on the internet.

Merged the five back into chronological position — placement taken from
`git log --reverse`, not from the dates in the headings, so they sit exactly where
their commits do: two between "the tablet band" and "`.card__jump` down to 50px",
three between that and "route off the copy". 62 entries → **67**. Then deleted the
fork.

**The habit that caused it:** an append with a relative path is only correct from
one directory. Worth using an absolute path for this file, or checking `pwd`
first.

### README corrections

Claims that had gone stale, each checked against the code rather than assumed:

| | was | now |
| --- | --- | --- |
| assets manifest | "legacy stills, 20 network photographs" | ten `net-01..net-10`; the legacy stills are deleted |
| generator | "FAL for the hero, Perth and the lid code" | one FAL entry (the street still); the lid-code asset and entry are gone |
| section table — Since 1890 | "the tiffin handover, a loaded handcart, a crowd of carriers" | the carrier with bags, a felicitation, a medalled line-up |
| section table — the waitlist | "20 real photographs… and a privacy note" | 10 photographs; the note was removed on request |
| section table — the run | pins "unlabelled on desktop" | unlabelled everywhere, and phones have their own unpinned route |
| section table — hero | "countdown in the nav row" | nav row on desktop, copy column below 900 |
| hero floor table | `≤700` floor 570 | ≤340 keeps 570, 341–700 is 540 |
| `.intro` tuck | `calc(-22% + 6px)` | `calc(10px - max(620px, 121.95%) * 0.2)`, with why the percentage-of-width version drifted |
| mosaic durations | 90s/92s, "bitten twice" | 43s/49s, bitten four times, with the full derivation |
| mosaic pause | "hover and `:focus-within` pause both tracks" | behind `(hover: hover) and (pointer: fine)`, with the touch-tap failure |

### CONTEXT.md

Untouched since 24 August, and its "verification habits" section had advice I have
since disproved. Rewritten: the iframe harness (and why injecting
`scrollbar-width: none` after load invalidates the measurement); that the Browser
pane never fires rAF/IO/ResizeObserver because `document.hidden` is permanently
true there, so scroll-driven work cannot be verified end-to-end in it; the
render-twice-and-difference method for text contrast, with the two wrong figures
it replaced; and that a probe re-implementing the page's own formula cannot catch
a bug in that formula. Also corrected the countdown's position, the dish rail's
provenance, the `LOOK`/`LOOK_STILL` steam trap, the "delete the entry with the
file" rule, and the claim that `python3 -m http.server` fails here — it works now
(verified: a fresh one answers 200; it binds `::1`, so probe URLs need
`localhost`).
